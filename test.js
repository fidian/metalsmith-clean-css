...
function callPlugin(plugin, files) {
    return new Promise((resolve, reject) => {
        plugin(files, { _directory: path.resolve(__dirname, "fixture") }, (errors) => {
            if (errors) {
                reject(errors);
            }
...
test(
	...
	t => {
        ...
        return callPlugin(metalsmithCleanCSS({cleanCSS: {level: {1: {specialComments: 0}}}}), files).then(() => {
            t.deepEqual(files, {
                'main.css': {
...
test('metalsmith-clean-css should forward clean-css errors', t => {
    ...
    return callPlugin(metalsmithCleanCSS({cleanCSS: {inline: 'all'}}), files).then(() => {
...
test(
	...
	t => {
        ...
        return callPlugin(metalsmithCleanCSS({sourceMap: true, sourceMapInlineSources: true}), files).then(() => {
            t.true(Boolean(files['main.css'].sourceMap));
            t.deepEqual(Object.keys(files), ['main.css']);
        });
	}
